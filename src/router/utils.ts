...
const _getRouteName = (name: string) => name?.replace('Page', '');

const _nativeUrl = ( fileName: string, path = '**') => `../views/${path}/${fileName}`;

const _getRoutesFlat = (indexPageName: string) => {
  const pages = import.meta.glob('../views/**/page.ts', {
    eager: true,
    import: 'default'
  });
  const pageComponent = import.meta.glob('../views/**/index.vue');
  const resources = import.meta.glob('../views/**/resources.ts', { import: 'default' });
  ...
  return Object.entries(pages).map(([importPath, config]) => {
    ...
    path = path === `/${upperCamel2Kebab(indexPageName)}` ? '/' : path;
...
const _createRoutes = (routesFlat: any[]) => {
  const nodeMap = new Map<string, any>(routesFlat.map(item => [item.route.name, item.route]));
